...
class Channel(Environment):
    ...
    def __generate_channels__(
        ...
        if (self.H_BS is None) or (self.h_Sk is None) or (self.h_Bk is None):
            ...
            G_Sk = (self.wavelength / (4 * pi)) * sqrt(gamma) * (torch.maximum(self.Phi_Sk, torch.zeros(K, N, device=self.device)) / (self.D_Sk / self.Dc_Sk)) ** q  # (K, N)
            self.h_Sk = G_Sk * (
            ...
            G_BS = (self.wavelength / (4 * pi)) * sqrt(gamma) * (torch.maximum(self.Phi_BSc, torch.zeros(N, M, device=self.device)) / (self.D_BS / self.Dc_BcSc)) ** q  # (N, M)
            self.H_BS = G_BS * (
    ...
    def __generate_channel_stochastic_components__(self, BS_Tx_power=-7):
        ...
        if (self.u_Bk is None) or (self.u_Sk is None) or (self.u_BS is None):
            ...
            mean_SNR = BS_Tx_power - noise - 10 * log10(M * K)
            self.mean_SNR = 10 ** (mean_SNR / 10)
            self.AWGN_norm = torch.view_as_complex(torch.normal(torch.zeros(size=(K, 1, 2), device=self.device), 1 / sqrt(2), generator=self.generator)) / sqrt(10 ** (mean_SNR / 10))
...
if __name__ == "__main__":
    ...
    for i in tqdm(range(n_iter)):
        ...
        W = beamformer.MRT(H)
        Pr_BS_RIS_UE = torch.abs(torch.einsum("kn,zn,nm,zkm->zk", h_Sk, Z * Theta, h_BS, W))
        Pr_BS_UE = torch.abs(torch.einsum("km,zkm->zk", h_Bk, W))
    ...
    ax[0].plot(range(n_env), Pr_BS_RIS_UE_avg.cpu().numpy(), color="b", label="BS-RIS-UE")
    ax[0].plot(range(n_env), Pr_BS_UE_avg.cpu().numpy(), color="r", label="BS-UE")
    # ax.hlines(Pr_BS_UE.cpu().numpy(), 0, n_env - 1, colors="r", label="BS-UE")
    ax[0].legend(loc="best")
    ax[0].set_title("Channel Gain")
    ax[0].set_xlabel("beam number")
    ax[0].set_ylabel("Channel gain (dB)")
    ax[0].grid()

    ax[1].plot(range(n_env), Pr_BS_RIS_UE_avg.cpu().numpy() - Pr_BS_UE_avg.cpu().numpy(), label="BS-RIS-UE/BS-UE")
    ax[1].set_title("BS-RIS-UE/BS-UE")
    ax[1].set_xlabel("beam number")
    ax[1].set_ylabel("Ratio (dB)")
    ax[1].grid()

    ax[2].plot(range(n_env), SINR_avg.cpu().numpy(), label="SINR")
    ax[2].set_title("SINR")
    ax[2].set_xlabel("beam number")
